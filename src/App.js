    ...
    <div style={{minHeight:"100vh",background:C.bg,display:"flex",alignItems:"center",justifyContent:"center",fontFamily:"system-ui,-apple-system,sans-serif",padding:20}}>
      <div style={{width:"100%",maxWidth:380}}>
        <div style={{textAlign:"center",marginBottom:32}}>
          ...
          <h1 style={{color:C.text,margin:"0 0 4px",fontSize:24,fontWeight:800}}>Clínica</h1>
...
function AgendaView({reservas,setReservas,users,config,currentUser,isManager}) {
  ...
  const remover=(id)=>{if(window.confirm("Remover esta reserva?"))setReservas(prev=>prev.filter(r=>r.id!==id));}; // eslint-disable-line no-restricted-globals
...
function ProfissionaisView({users,setUsers}) {
  ...
  const remover=(id)=>{if(window.confirm("Remover este profissional?"))setUsers(prev=>prev.filter(u=>u.id!==id));}; // eslint-disable-line no-restricted-globals
